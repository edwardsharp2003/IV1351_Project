    ...
    def _compute_teaching_cost_ui(self, course_code: str = None, study_year: str = None):
        ...
        try:
            ...
            if result:
                ...
                print(
                    f"{'Course Code':<15} {'Course Instance ID':<20} {'Period':<8} {'Planned Cost (KSEK)':<25} {'Actual Cost (KSEK)':<25}")
                print("-" * 93)
                print(
                    f"{result.course_code:<15} {result.course_instance_id:<20} {result.period:<8} {result.planned_cost_ksek:<25.2f} {result.actual_cost_ksek:<25.2f}")
                print("-" * 93)
            else:
    ...
    def _modify_student_count_ui(self):
        ...
        try:
            ...
            if success:
                ...
                print(
                    f"Failed to update student count for {course_code} ({study_year}). Course instance might not exist.")
        except Exception as e:
        ...
        try:
            ...
            if activities:
                ...
                for row in activities:
                    print(
                        f"{row['activity_name']:<15} {row['course_code']:<10} {row['study_year']:<6} {row['planned_hours']:<6} {row['employee_id']:<8}")
                print("-" * 50)
            else:
